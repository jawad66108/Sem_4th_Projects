...
// GET student's latest SGPA from CGPA_RECORDS
router.get("/my/gpa/:student_id", async (req, res) => {
  let connection;
  try {
    connection = await getConnection();
    const result = await connection.execute(
      `SELECT sgpa, cgpa, semester, recorded_at
             FROM cgpa_records
             WHERE student_id = :id
             ORDER BY recorded_at DESC
             FETCH FIRST 1 ROWS ONLY`,
      { id: req.params.student_id },
      { outFormat: require("oracledb").OUT_FORMAT_OBJECT }, // ← add this
    );
